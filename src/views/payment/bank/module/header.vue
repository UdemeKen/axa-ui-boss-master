  ...
  <div class="head-container">
    ...
    <blockquote class="my-blockquote" size="mini" style="padding-bottom: 5px;">
      ...
      <div style="display: inline-block;margin: 0px 2px; float:right">
        <el-button
          v-if="isAuth('bank:add')"
          class="filter-item"
          size="mini"
          type="primary"
          icon="el-icon-plus"
          @click="$refs.form.dialog = true">新增银行</el-button>
        <el-button v-if="isAuth('bank:disabled')" :disabled="this.$parent.selections.length <= 0" class="filter-item" size="mini" type="warning" icon="el-icon-view" @click="disabledHandle(true)">批量禁用</el-button>
        <el-button v-if="isAuth('bank:disabled')" :disabled="this.$parent.selections.length <= 0" class="filter-item" size="mini" type="success" icon="el-icon-view" @click="disabledHandle(false)">批量启用</el-button>
...
import { disBank } from '@/api/payment'
...
export default {
  ...
  methods: {
    ...
    disabledHandle(dis) {
      ...
      const names = this.$parent.selections.map(item => { return item.bankName })
