<template>
  <div class="head-container">
    ...
    <blockquote class="my-blockquote" size="mini" style="padding-bottom: 5px;">
      ...
      <el-select v-model="query.methodCode" filterable clearable placeholder="支付产品" class="filter-item" style="width: 120px" @change="toQuery">
      ...
      <div style="display: inline-block;margin: 0px 2px; float:right">
        <el-button
          ...
          type="primary"
          icon="el-icon-plus"
          @click="$refs.form.dialog = true">新增支付产品</el-button>
        <el-button v-if="isAuth('pm:disabled')" :disabled="this.$parent.selections.length <= 0" class="filter-item" size="mini" type="warning" icon="el-icon-view" @click="disabledHandle(true)">批量禁用</el-button>
        <el-button v-if="isAuth('pm:disabled')" :disabled="this.$parent.selections.length <= 0" class="filter-item" size="mini" type="success" icon="el-icon-view" @click="disabledHandle(false)">批量启用</el-button>
