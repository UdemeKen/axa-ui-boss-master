  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="data" size="small" border style="width: 100%;">
      ...
      <el-table-column label="API接口" min-width="80px" align="center">
        <template slot-scope="scope">
          <span v-if="scope.row.methodType == 3">交易回调</span>
      ...
      <el-table-column :show-overflow-tooltip="true" prop="notifyUrl" label="回调地址" min-width="140px"/>
      <el-table-column label="回调参数" min-width="200px">
        <template slot-scope="scope">
          <el-popover
            ...
            width="600"
            trigger="click">
            <a slot="reference" style="min-width:180px;overflow: hidden; text-overflow: ellipsis; display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp: 1;">{{ scope.row.notifyJson }}</a>
          </el-popover>
        </template>
      </el-table-column>
      <el-table-column label="商户返回" min-width="120px">
        <template slot-scope="scope">
          <el-popover
            ...
            width="420"
            trigger="click">
            <a slot="reference" style="min-width:100px;overflow: hidden; text-overflow: ellipsis; display: -webkit-box; -webkit-box-orient: vertical; -webkit-line-clamp: 1;">{{ scope.row.notifyResult }}</a>
          </el-popover>
        </template>
      </el-table-column>
      <el-table-column label="回调状态" align="center" min-width="120px">
        <template slot-scope="scope">
          <span v-if="scope.row.successed" style="color: #67C23A;">成功</span>
          <span v-else style="color: #F56C6C">商户返回错误</span>
        </template>
      </el-table-column>
...
export default {
  ...
  data() {
    return {
      delLoading: false, sup_this: this, pageNum: 1, selections: [], mchOptions: [], pmOptions: [], accOptions: [], countryOptions: countryOptions
    }
  },
  created() {
    this.$nextTick(() => {
