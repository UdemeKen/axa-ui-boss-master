<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="data" size="small" border style="width: 100%;">
      ...
      <el-table-column prop="tradeSysIncomeAmount" label="代收系统抽成(元)" min-width="90px">
        <template slot-scope="scope">
          {{ mathDiv(scope.row.tradeSysIncomeAmount, 100) }}
        </template>
      </el-table-column>
      ...
      <el-table-column prop="drawSysIncomeAmount" label="代付系统抽成(元)" min-width="90px">
        <template slot-scope="scope">
          {{ mathDiv(scope.row.drawSysIncomeAmount, 100) }}
        </template>
      </el-table-column>
    </el-table>
    <!--分页组件-->
